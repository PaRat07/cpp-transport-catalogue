...
std::tuple<std::string_view, size_t, size_t, double> TransportCatalogue::GetDataForBus(std::string_view bus) const {
    const Bus* bus_ptr = SearchBusByName(bus);
    if (bus_ptr == nullptr) {
        return tuple(bus, 0, 0, 0);
    }
    ...
    return { bus, bus_ptr->stops.size(), unique_stops_for_bus_.at(bus_ptr).size(), route_length };
}

std::tuple<bool, std::string_view, std::set<std::string_view, std::less<>>> TransportCatalogue::GetDataForStop(std::string_view stop_name) const {
